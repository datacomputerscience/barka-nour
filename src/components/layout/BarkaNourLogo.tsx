...
export function BarkaNourLogo({ size = 'default', variant = 'full', className }: { size?: 'sm' | 'default' | 'lg', variant?: 'full' | 'icon', className?: string }) {
  ...
  return (
    <div className={cn("flex items-center gap-2.5 select-none", className)}>
      <div className="relative flex items-center justify-center rounded-xl bn-gradient shadow-sm" style={{ width: s.icon, height: s.icon }}>
        <svg width={s.icon * 0.6} height={s.icon * 0.6} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <circle cx="12" cy="12" r="3" fill="white" fillOpacity="0.95"/>
          <circle cx="12" cy="12" r="1" fill="#854d0e"/>
        </svg>
      </div>
      {variant === 'full' && (
        <div className="flex flex-col leading-none">
          <span className={cn("font-display font-bold tracking-tight text-ink-900", s.text)}>Barka Nour</span>
          {size !== 'sm' && <span className={cn("font-medium tracking-widest text-bn-600 uppercase -mt-0.5", s.sub)}>بركة نور • Artisanat Tunisien</span>}
        </div>
      )}
    </div>
  )
